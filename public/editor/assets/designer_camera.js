pc.script.create( "designer_camera", function (context) {
    ...
    var DesignerCamera = function (entity) {
        ...
        this.isUserCamera = !pc.string.startsWith(this.entity.getPath(), 'Designer/Local/Designer/');
    };
    ...
    DesignerCamera.prototype.frameSelection = function () {
        ...
        pc.designer.api.setCameraTransformComponent(this.entity, 'position', this.entity.getLocalPosition(), false);
    };
    ...
    */
    ...
    DesignerCamera.prototype.orbit = function (rotation) {
        ...
        pc.designer.api.setCameraTransformComponent(this.entity, 'position', position, false);
        pc.designer.api.setCameraTransformComponent(this.entity, 'rotation', angles, false);
    };
    ...
    DesignerCamera.prototype.toolsUpdate = function () {
        ...
        if (transition.active) {
            ...
            setTimeout(function () {
                // if the transition is no longer active then make the camera
                // position change undoable
                pc.designer.api.setCameraTransformComponent(this.entity, 'position', this.entity.getLocalPosition(), false);
    ...
    // Saves the original position of the camera
    // and starts a timeout to send an undoable command to the designer
    // after a little while
    DesignerCamera.prototype.recordUndoablePosition = function () {
        if (this.isUserCamera)  {
            if (!this.positionDirty) {
                this.positionDirty = true;
                this.originalPosition.copy(this.entity.getLocalPosition());
            }

            this.resetUndoableTimeout();
        }

    };

    // Saves the original rotation of the camera
    // and starts a timeout to send an undoable command to the designer
    // after a little whil
    DesignerCamera.prototype.recordUndoableRotation = function () {
        if (this.isUserCamera)  {
            if (!this.rotationDirty) {
                this.rotationDirty = true;
                this.originalRotation.copy(this.entity.getLocalEulerAngles());
            }
    ...
    // Resets the timeout that sends an undoable transform
    // to the designer
    DesignerCamera.prototype.resetUndoableTimeout = function () {
        if (this.undoableTimeout) {
            clearTimeout(this.undoableTimeout);
        }

        this.undoableTimeout = setTimeout(function () {
            // here we're sending two separate undoable actions
            // so the user needs to undo twice to get to the original transform
            if (this.positionDirty) {
                pc.designer.api.setCameraTransformComponent(this.entity, 'position', this.entity.getLocalPosition(), true, this.originalPosition);
                this.positionDirty = false;
            }

            if (this.rotationDirty) {
                pc.designer.api.setCameraTransformComponent(this.entity, 'rotation', this.entity.getLocalEulerAngles(), true, this.originalRotation);
                this.rotationDirty = false;
            }
        }.bind(this), 500);
    };
